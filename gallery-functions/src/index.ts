 ...
 */
export const PBvotePrompt = onCall(async (request) => {
  const { promptId, userId, action } = request.data || {};

  if (!promptId || typeof promptId !== "string" || promptId.trim().length === 0) {
    throw new HttpsError("invalid-argument", "El promptId es requerido y debe ser una cadena de texto válida.");
  }
  if (!userId || typeof userId !== "string" || userId.trim().length === 0) {
    throw new HttpsError("invalid-argument", "El userId es requerido y debe ser una cadena de texto válida.");
  }
  if (action !== "upvote" && action !== "downvote") {
    throw new HttpsError("invalid-argument", "El campo action es obligatorio y debe ser 'upvote' o 'downvote'.");
  }
  ...
  try {
    ...
    await db.runTransaction(async (transaction) => {
      ...
      if (!userDoc.exists) {
        throw new HttpsError("not-found", `No se encontró ningún participante registrado con el ID: ${userId}`);
      }

      // 2. Obtener y validar el documento del prompt (la imagen)
      const promptDoc = await transaction.get(promptRef);
      if (!promptDoc.exists) {
        throw new HttpsError("not-found", `No se encontró ninguna imagen en la galería con el ID: ${promptId}`);
      }

      const promptData = promptDoc.data();
      if (promptData?.status !== "completed") {
        throw new HttpsError("failed-precondition", "Solo se puede votar por imágenes cuyo estado de procesamiento sea 'completed'.");
      }

      // Obtener el array de prompts votados por el usuario
      const userData = userDoc.data();
      let votedPromptIds: string[] = userData?.votedPromptIds || [];

      if (action === "upvote") {
        // Verificar duplicados de voto
        if (votedPromptIds.includes(promptId)) {
          throw new HttpsError("already-exists", "Ya has votado por esta imagen.");
        }
        // Validar límite máximo de 3 votos
        if (votedPromptIds.length >= 3) {
          throw new HttpsError("resource-exhausted", "Límite de votos alcanzado. No puedes votar por más de 3 imágenes.");
        }
        ...
        if (!votedPromptIds.includes(promptId)) {
          throw new HttpsError("failed-precondition", "No tienes registrado un voto activo para esta imagen, por lo que no lo puedes retirar.");
        }
    ...
    throw new HttpsError("internal", `Fallo interno al procesar el voto: ${error?.message || error}`);
  }
});
 ...
 */
export const PBgenerateImageTrigger = onDocumentCreated("prompts/{promptId}", async (event) => {
  ...
  const docsToProcess: { id: string; ref: admin.firestore.DocumentReference; promptText: string }[] = [];
    ...
    try {
      ...
      if (ai) {
        try {
          ...
          const isCredsError =
            apiError?.message?.includes("Default Credentials") ||
